...
for lib in libospray.so libospray_module_cpu.so libospray_module_mpi_offload.so libospray_module_mpi_distributed_cpu.so ; do
  echo "checking $lib..."
  check_symbols $lib GLIBC   2 32 0
  check_symbols $lib GLIBCXX 3 4 26
...
cp ./ospray-2.11.0.x86_64.linux.tar.gz /output/ospray-2.11.0.x86_64.linux_sparse_sampling.tar.gz
